...
class TestWildcards(TestCase):
    ...
    def test_optional(self):
        fun = TemplateEvaluator().parse('{optional<{two<{three}>}>?} {foo} {bar}')
        obj1 = fun('foo bar')
    ...
    def test_all_matches(self):
        template = TemplateEvaluator(all_matches=True)('{word1} {word2?} {word3?} {word4}')
        objs = template('three optional words')
        assert len(objs) == 4
    
    def test_repeat_match(self): 
        template = TemplateEvaluator()('{word1} {foos<\s*foo>!} {word2}')
        obj = template('word1 foo foo foo foo foo word2')
        assert len(obj.foos) == 5

    def test_loose_repeat_match(self):
        template = TemplateEvaluator()('{word1} {foos<foo>~!} {word2}')
        obj = template('word1 foo | foo | foo | foo | not a foo | foo word2')
